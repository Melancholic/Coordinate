class UsersController < HTTPApplicationController
  ...
  before_action :signed_in_user, only: [:show, :edit, :update, :destroy, :verification, :sent_verification_mail] # in app/helpers/session_helper.rb
  before_action :verificated_user, only: [:destroy, :show]
  ...
  skip_before_action :user_is_agreed, only: [:do_agree]
  ...
  def do_agree
    if signed_in?
      @user = current_user
      if @user.update_attributes(is_agreed: user_params[:is_agreed])
        redirect_to_target_url(root_path)
      else
  ...
  def user_params
    params.require(:user).permit(:login, :email, :password, :password_confirmation,
                                 :time_zone, :captcha, :captcha_key, :locale, :is_agreed,
